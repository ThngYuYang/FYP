   ...
   private static List<DTO> ToStatic<DTO>(this DataTable dt)
   {
      ...
      foreach (DataRow row in dt.Rows)
      {
         ...
         foreach (DataColumn column in dt.Columns)
         {
            ...
            {
               // Debug.WriteLine(row[column].GetType() + " " + Prop?.PropertyType + " " + Nullable.GetUnderlyingType(Prop?.PropertyType)); 
               if (row[column].GetType() == Prop?.PropertyType ||
                   row[column].GetType() == Nullable.GetUnderlyingType(Prop?.PropertyType))
